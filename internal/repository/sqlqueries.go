...
const (
	...
	QSessionInfo = `
		SELECT 
			...
			COALESCE(DH.NAME, 'Не выбрана') 																	database_name,
			COALESCE(DU.NAME, 'Не выбрана')																		username_name,
			COALESCE(DP.NAME, 'Не выбрана')																		password_name,
			...
			S.ERRORS, S.WORKER_COUNT, S.MODULE_ID, M.TYPE module_type
		FROM SESSION S
	...
	QGetPercentProtocol = `
		...
		FROM ( 
			SELECT 
				(SELECT COUNT(1) FROM DATABASE_LINK_DATA WHERE DATABASE_ID = S.DATABASE_ID) H_COUNT, 
				(SELECT COUNT(1) FROM DATABASE_LINK_DATA WHERE DATABASE_ID = S.USERNAME_ID) U_COUNT, 
				(SELECT COUNT(1) FROM DATABASE_LINK_DATA WHERE DATABASE_ID = S.PASSWORD_ID) P_COUNT, 
	...
	QInsertSession = `
		INSERT INTO SESSION(NAME, STATUS, WORKER_COUNT, MODULE_ID)
		VALUES($1, 'ST1', 100, $2);
	`
	...
	QDeleteSession = `
		...
		DELETE FROM SESSION_DATA WHERE SESSION_ID = $2;
